...
void blas_forward_conv_layer(layer *current)
{
	...
	im2col_fct(c_param->im2col_input, current->input, 
		flat_prev_size, flat_nb_area * c_param->flat_f_size, 
	...
	cblas_sgemm(CblasColMajor, CblasTrans, CblasNoTrans, net->batch_size 
		* flat_nb_area, c_param->nb_filters, c_param->flat_f_size, 1.0f, 
		/*A*/ c_param->im2col_input, c_param->flat_f_size, /*B*/ c_param->filters, c_param->flat_f_size, 0.0f, 
		/*C*/ current->output, net->batch_size * flat_nb_area);
...
void blas_backward_conv_layer(layer *current)
{
	...
	if(current->previous != NULL)
	{
		...
		cblas_sgemm(CblasColMajor, CblasTrans, CblasNoTrans,
			flat_prev_size*net->batch_size, c_param->prev_depth, 
			spatial_f_size*c_param->nb_filters, 1.0f, /*A*/c_param->im2col_delta_o, 
			spatial_f_size*c_param->nb_filters, /*B*/c_param->rotated_filters, 
			spatial_f_size*c_param->nb_filters, 0.0f, /*C*/c_prev_delta_o, 
			flat_prev_size*net->batch_size);
