...
__global__ void quadratic_output_error_kernel(real *delta_o, real *output, real *target, 
...
__global__ void softmax_output_error_kernel(real *delta_o, real *output, real *target, int len, int dim, int size);
...
{	
	...
	quadratic_output_error_kernel<<< cu_blocks, cu_threads >>>(current->delta_o, current->output,
...
__global__ void quadratic_output_error_kernel(real *delta_o, real *output, real *target, int len, int dim, int size)
{
	int i = blockIdx.x*blockDim.x + threadIdx.x;
	int pos;
...
void cuda_softmax_output_error(layer *current)
{
	...
	softmax_output_error_kernel<<< cu_blocks, cu_threads >>>(current->delta_o, current->output, target, (param->dim+1)*length, param->dim, (param->dim+1)*batch_size);
