...
void norm_create(network *net, layer *previous, const char *norm_type, int group_size, int set_off, FILE *f_load, int f_bin)
{
	int i;
	...
	{
		if(net->net_layers[i]->dropout_rate > 0.01f)
		{
			printf("\nERROR: A normalization layer cannot be set if dropout is used in any previous layer in the network due to variance shift.\n");
			printf("Consider removing the dropout in all previous layers or not having any normalization after the last dropout.\n");
			exit(EXIT_FAILURE);
		}
	}
	...
	mem_approx += 2*n_param->nb_group*net->batch_size;
...
\t Nb. params: %d, Approx layer RAM/VRAM requirement: %d MB\n",
		...
		2*n_param->nb_group, (int)(mem_approx/1000000));
	net->total_nb_param += 2*n_param->nb_group;
...
void norm_save(FILE *f, layer *current, int f_bin)
{
	...
	else
	{
		fprintf(f,"N ");
		print_norm_type(f, current, f_bin);
		fprintf(f, "S%d_O%d\n", n_param->group_size, n_param->set_off);
	}
...
void norm_load(network *net, FILE *f, int f_bin)
{
	...
	norm_create(net, previous, norm, group_size, set_off, f, f_bin);
}
