...
cnn.init(in_dim=i_ar([image_size,image_size]), in_nb_ch=3, out_dim=1+max_nb_obj_per_image*(7+1),
...
nb_yolo_filters = cnn.set_yolo_params(nb_box = nb_box, nb_class = nb_class, max_nb_obj_per_image = max_nb_obj_per_image,
				...
				rand_startup = nb_images_per_iter*20, rand_prob_best_box_assoc = 0.05, class_softmax = 1, error_type = "natural", no_override = 1)
...
for block in range(start_block,int(total_epochs/nb_epoch_per_augm)):
	...
	if((block+1)*nb_epoch_per_augm <= 20):
		...
		loc_wd = weight_decay
	elif((block+1)*nb_epoch_per_augm <= 2500):
		loc_lr = learning_rate
		loc_wd = weight_decay
	elif((block+1)*nb_epoch_per_augm <= 3500):
		loc_lr = learning_rate*0.1
		loc_wd = weight_decay#*0.5
	else:
		loc_lr = learning_rate*0.01
		loc_wd = weight_decay#*0.2
	
	cnn.train(nb_iter=nb_epoch_per_augm, learning_rate=loc_lr, shuffle_every=0, control_interv=1, \
			momentum=0.9, lr_decay=0.0, weight_decay=loc_wd, save_every=100, silent=0, save_bin=1, TC_scale_factor=96.0)
	if(block == 0):
		cnn.perf_eval()
	t.join()
	...
	if((block+1)*nb_epoch_per_augm >= 50 and np.mod((block+1)*nb_epoch_per_augm,1) == 20):
		...
		pred_postprocess((block+1)*nb_epoch_per_augm, obj_threshold=0.03, class_soft_limit=0.3, nms_threshold_same=0.4, nms_threshold_diff=0.9)
